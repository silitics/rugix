...
if test "${bootpart}" = ""; then
    setenv bootpart 2
	echo "set default bootpart to ${bootpart}"
fi

if test -e ${devtype} ${devnum}:${bootpart} ${prefix}armbianEnv.txt; then
	load ${devtype} ${devnum}:${bootpart} ${scriptaddr} ${prefix}armbianEnv.txt
...
if test -e ${devtype} ${devnum}:${bootpart} ${prefix}zImage; then
	# legacy kernel boot
	...
	load ${devtype} ${devnum}:${bootpart} ${k_addr} boot/zImage
	load ${devtype} ${devnum}:${bootpart} ${dtb_loadaddr} boot/dtb/${fdtfile}
	load ${devtype} ${devnum}:${bootpart} ${initrd_loadaddr} boot/uInitrd
	...
	for overlay_file in ${overlays}; do
		if load ${devtype} ${devnum}:${bootpart} ${scriptaddr} ${prefix}dtb/amlogic/overlay/${overlay_prefix}-${overlay_file}.dtbo; then
	...
	for overlay_file in ${user_overlays}; do
		if load ${devtype} ${devnum}:${bootpart} ${scriptaddr} ${prefix}overlay-user/${overlay_file}.dtbo; then
	...
	if test "${overlay_error}" = "true"; then
		...
		if load ${devtype} ${devnum}:${bootpart} ${scriptaddr} ${prefix}dtb/amlogic/overlay/${overlay_prefix}-fixup.scr; then
			echo "Applying kernel provided DT fixup script (${overlay_prefix}-fixup.scr)"
			source ${scriptaddr}
		fi
		if test -e ${devtype} ${devnum}:${bootpart} ${prefix}fixup.scr; then
			load ${devtype} ${devnum}:${bootpart} ${scriptaddr} ${prefix}fixup.scr
